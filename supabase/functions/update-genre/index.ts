...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const getData = async (supabase, genre) => {
        
      var id = genre.id;
      var json = JSON.stringify(genre);
      var name = genre.name;
      var slug = genre.slug;
      ...
      const { data, error } = await supabase
          .from('genre_enum')
          .upsert([
            { id: id, name: name, slug: slug, data: json }
          ])
          .select();
    ...
    var log, errors = await ImportData("genres", "genre_enum", getData, supabase);
...
/* To invoke locally:
  ...
  curl -i --location --request POST 'http://127.0.0.1:54321/functions/v1/update-genre' \
